...
There are two tasks in this challenge: *Object-based Semantic Mapping / SLAM*, and *Scene Change Detection*.
<!-- **We are working towards presenting this new challenge in November 2018 during our [workshop](iros2019) at IROS.** -->

Key features of the challenge:
 * We make heavy use of the Nvidia Isaac simulator for this challenge.
 * We provide a simple API to interface the simulated robot, following the OpenAI Gym API.
 * Each task in the challenge has three difficulty levels, each increase making the system more akin to a real robot
 * The best teams are invited to execute their code on a real robot in our lab. This robot can be controlled using the same API as the simulated robot using dockerized code.
